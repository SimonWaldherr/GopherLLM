...
func TestArchitectureSupportedCoversImplementedLoaders(t *testing.T) {
	for _, arch := range []string{"llama", "llama2", "llama3", "mistral", "mistral3", "mixtral", "qwen2", "qwen2moe", "qwen3", "qwen3moe", "qwen35", "qwen35moe", "deepseek2", "kimi_k2", "phi3", "granite", "exaone", "exaone4", "smollm3", "olmo2", "internlm2", "stablelm", "gpt-oss", "gemma", "gemma2", "gemma3", "gemma4", "bert", "nomic-bert"} {
		if !ArchitectureSupported(arch) {
			t.Fatalf("ArchitectureSupported(%q) = false, want true", arch)
		}
	}
	for _, arch := range []string{"phi2"} {
		if ArchitectureSupported(arch) {
