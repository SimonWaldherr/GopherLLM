// Package safetensors reads dense tensors from the safetensors container
// format. It is independent of GopherLLM's model runtime so model-specific
// loaders can share the same parser without depending on the root package.
package safetensors
...
// DType identifies the stored numeric representation of a tensor.
type DType string
...
func Parse(data []byte) (*SafetensorsFile, error) {
	if len(data) < 8 {
...
func (s *SafetensorsFile) F32(name string) ([]float32, error) {
	...
	case F16:
		...
		for i := range out {
			out[i] = numeric.F16ToF32(binary.LittleEndian.Uint16(raw[i*2:]))
		}
	case BF16:
		if len(raw) != info.Numel*2 {
