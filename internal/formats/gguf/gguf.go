...
// TypeFromID maps a raw GGUF dtype id to a supported GGMLType, folding
// encodings this package does not recognize into GGMLTypeUnknown.
func TypeFromID(v uint32) GGMLType {
	switch GGMLType(v) {
...
func Parse(data []byte) (*GGUFFile, error)      { return parseGGUF(data) }
func ParseQuiet(data []byte) (*GGUFFile, error) { return parseGGUF(data) }
...
func parseGGUF(data []byte) (*GGUFFile, error) {
	...
	for range int(nTensors) {
		...
		tensors = append(tensors, TensorInfo{Name: name, Dims: dims, DType: TypeFromID(dt), Offset: off})
	}
	alignment := 32
