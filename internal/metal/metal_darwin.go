...
static bool gllm_metal_init_q4k(void) {
	@autoreleasepool {
		...
		id<MTLFunction> fn = [library newFunctionWithName:@"gllm_q4k_matvec"];
		if (fn == nil) {
			strncpy(gllm_error, "failed to load Q4_K Metal function", sizeof(gllm_error) - 1);
			[library release];
			return false;
		}
		gllm_q4k_pipeline = [gllm_device newComputePipelineStateWithFunction:fn error:&error];
		[fn release];
		[library release];
		if (gllm_q4k_pipeline == nil) {
			gllm_set_error(@"failed to create Q4_K Metal pipeline", error);
			return false;
		}
		return true;
	}
...
static void* gllm_metal_new_q4k(const void* data, long len, int rows, int cols) {
	@autoreleasepool {
...
func PrepareQ4K(data []byte, rows, cols int) *Weight {
	if rows <= 0 || cols <= 0 || cols%256 != 0 || len(data) == 0 {
		return nil
	}
	ptr := C.gllm_metal_new_q4k(unsafe.Pointer(&data[0]), C.long(len(data)), C.int(rows), C.int(cols))
	if ptr == nil {
		return nil
	}
