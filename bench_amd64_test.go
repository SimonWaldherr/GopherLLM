...
func BenchmarkDotMXFP4Q8KRow_4096(b *testing.B) {
	if !hasAVX2 || !hasF16C {
		b.Skip("AVX2+F16C required")
	}
	rng := rand.New(rand.NewSource(13))
	row := randomMXFP4Row(rng, 4096)
	x := randomVec(rng, 4096)
	q8 := make([]int8, 4096)
	sc := make([]float32, 4096/256)
	q8kQuantize(&x[0], &q8[0], &sc[0], 4096/256)
	...
	b.SetBytes(int64(len(row)))
	for b.Loop() {
		_ = mxfp4DotQ8KRow(&row[0], &q8[0], &sc[0], 4096/256)
	}
}

func BenchmarkDotMXFP4Float_4096(b *testing.B) {
	rng := rand.New(rand.NewSource(13))
	row := randomMXFP4Row(rng, 4096)
	x := randomVec(rng, 4096)
	b.ReportAllocs()
...
func BenchmarkDotQ4_0Q8KRow_4096(b *testing.B) {
	if !hasAVX2 || !hasF16C {
		b.Skip("AVX2+F16C required")
	}
	rng := rand.New(rand.NewSource(11))
	row := randomQ4_0Row(rng, 4096)
	x := randomVec(rng, 4096)
	scratch := []float32{}
	xs := fillQ4KXSums(x, 4096, &scratch)
	q8 := make([]int8, 4096)
	...
	b.SetBytes(int64(len(row)))
	for b.Loop() {
		_ = q4_0DotQ8KRow(&row[0], &q8[0], &sc[0], &xs[0], 4096/256)
	}
}

func BenchmarkDotQ4_0Float_4096(b *testing.B) {
	rng := rand.New(rand.NewSource(11))
	row := randomQ4_0Row(rng, 4096)
	x := randomVec(rng, 4096)
	b.ReportAllocs()
...
func BenchmarkDotQ4_1Q8KRow_4096(b *testing.B) {
	if !hasAVX2 || !hasF16C {
		b.Skip("AVX2+F16C required")
	}
	rng := rand.New(rand.NewSource(12))
	row := randomQ4_1Row(rng, 4096)
	x := randomVec(rng, 4096)
	scratch := []float32{}
	xs := fillQ4KXSums(x, 4096, &scratch)
	q8 := make([]int8, 4096)
	...
	b.SetBytes(int64(len(row)))
	for b.Loop() {
		_ = q4_1DotQ8KRow(&row[0], &q8[0], &sc[0], &xs[0], 4096/256)
	}
}

func BenchmarkDotQ4_1Float_4096(b *testing.B) {
	rng := rand.New(rand.NewSource(12))
	row := randomQ4_1Row(rng, 4096)
	x := randomVec(rng, 4096)
	b.ReportAllocs()
