...
func onlineAttentionF16(query []float32, keys, values []uint16, keyStride, valueStride, keyHeadDim, valueHeadDim, startT, endT int, scale, softcap float32, out []float32) {
	onlineAttentionF16WithSink(query, keys, values, keyStride, valueStride, keyHeadDim, valueHeadDim, startT, endT, scale, softcap, 0, 0, false, out)
}
...
func onlineAttentionF16WithSink(query []float32, keys, values []uint16, keyStride, valueStride, keyHeadDim, valueHeadDim, startT, endT int, scale, softcap, alibiSlope, sink float32, hasSink bool, out []float32) {
	span := endT - startT + 1
	if span <= 0 {
		return
	}
	...
	for t := startT; t <= endT; t++ {
		...
		scores[n] = dotF32F16(query, keys[kOff:kOff+keyHeadDim])*scale + alibiSlope*float32(t-endT)
		n++
	}
