...
// NOTE: deliberately a copy of the root package's gguf_synth_test.go. The
// builder needs gopherllm.GGMLType, so a shared fixture package would import
// the root package while the root package's own in-package tests import the
// fixture — an import cycle. Test-only duplication buys package independence.
//
...
func buildTinyLlamaGGUF() []byte { return buildTinyLlamaGGUFWithContext(1024) }

// buildTinyLlamaGGUFWithContext is the same synthetic model with a chosen
// declared context_length, so a test can make a history overflow the window
// without reaching into the Runner's unexported config.
func buildTinyLlamaGGUFWithContext(contextLength uint32) []byte {
	const (
		dim    = 8
		heads  = 2
		kv     = 2
	...
	kvs := []ggufKV{
		...
		{"llama.context_length", ggufU32, contextLength},
	...
	f32t := func(name string, rows, cols, seed int) ggufTensor {
		...
		return ggufTensor{name: name, dims: []uint64{uint64(cols), uint64(rows)}, dtype: gopherllm.GGMLTypeF32, data: f32Bytes(smallWeights(rows*cols, seed))}
	}
	vec := func(name string, n int) ggufTensor {
		return ggufTensor{name: name, dims: []uint64{uint64(n)}, dtype: gopherllm.GGMLTypeF32, data: f32Bytes(onesF32(n))}
	}
	tensors := []ggufTensor{
